...
class ICC_018:
	...
	"""
	play = BuffState(SELF, FRIENDLY_WEAPON, "ICC_018e")
...
class ICC_031:
	...
	"""
	events = SELF_DAMAGE.on(Buff(SELF, "ICC_031e"))
...
class ICC_067:
	...
	"""
	deathrattle = Find(CURRENT_PLAYER + CONTROLLER) | Summon(CONTROLLER, "ICC_900t")
...
class ICC_096:
	...
	"""
	play = Destroy(WEAPON + FRIENDLY_HAND), BuffState(SELF, WEAPON + FRIENDLY_HAND, "ICC_096e")
...
class ICC_097:
	...
	"""
	events = Death(FRIENDLY_WEAPON).on(Buff(SELF, "ICC_097e"))
...
class ICC_098:
	...
	"""
	play = Give(CONTROLLER, Copy(RANDOM(FRIENDLY + KILLED + MINION + DEATHRATTLE)))
...
class ICC_314:
	...
	events = OWN_TURN_END.on(
		Give(CONTROLLER, RandomCard(card_class=CardClass.DEATHKNIGHT))
	)
...
class ICC_481p:
	...
	"""
	activate = (COST(TARGET) >= 12) | Morph(TARGET, RandomMinion(cost=COST_ADD(TARGET)))
...
# 	"""
# 	TODO: Add hero_healed_this_turn_tag in player.py
...
class ICC_701:
	...
	"""
	play = Destroy(SPELL + IN_HAND + IN_DECK + (COST == 1))
...
class ICC_833e:
	...
	"""
	update = Refresh(FRIENDLY + MINION + ELEMENTAL, "ICC_833e2")

ICC_833e2 = buff(lifesteal=True)
# class ICC_833e2:
# 	"""
...
class ICC_833h:
	...
	"""
	activate = Hit(TARGET, 1), Dead(TARGET) & Summon(CONTROLLER, "ICC_833t")

ICC_837e = buff(cost=-2)
# class ICC_837e:
# 	"""
...
class ICC_849e:
	...
	"""
	events = OWN_TURN_BEGIN.on(Steal(OWNER))	
...
class ICC_854:
	...
	"""
	deathrattle = Give(CONTROLLER, RandomCard(card_class=CardClass.DEATHKNIGHT))
...
class ICC_900:
	...
	"""
	events = Death(FRIENDLY + MINION - SELF).after(Summon(CONTROLLER, "ICC_900t"))
...
# 	"""
# 	TODO: Add extra_end_turn_effects for player and EndTurn
...
class ICC_902:
	...
	"""
	update = Refresh(FRIENDLY_HERO_POWER, {GameTag.HEROPOWER_ADDITIONAL_ACTIVATIONS: SET(-2)})
...
class ICC_904e:
	...
	"""
	amount = Count(FRIENDLY + MINION + KILLED_THIS_TURN)
	atk = amount
	health = amount
...
class ICC_062:
	...
	"""
	deathrattle = Find(CURRENT_PLAYER + CONTROLLER) | GainArmor(CONTROLLER, 6)
...
class ICC_091:
	...
	"""
	play = Shuffle(CONTROLLER, Copy(FRIENDLY_HAND))
...
class ICC_238:
	...
	"""
	events = Play(CONTROLLER, FRIENDLY + MINION).after(Hit(Play.CARD, 1))
...
class ICC_041:
	...
	"""
	play = Hit(ALL_MINIONS, 1), DeadOne(ALL_MINIONS) & (Deaths(), CastSpell("ICC_041"))
...
class ICC_831:
	...
	"""
	play = Summon(CONTROLLER, Copy(FRIENDLY + MINION + KILLED + DEMON))
...
class ICC_841:
	...
	"""
	update = Refresh(SELF, buff="ICC_841e")
	class Hand:
		update = Refresh(SELF, buff="ICC_841e")
...
class ICC_903:
	...
	"""
	play = Destroy(TARGET).then(Buff(SELF, "ICC_903"))
...
class ICC_056:
	...
	"""
	play = Freeze(TARGET), Buff(TARGET, "ICC_056e")
...
# 	"""
# 	TODO: Add tag "overloaded_this_game" in player
...
# 	"""
#	events = Freeze(ALL_MINIONS - SELF).on(Give(CONTROLLER, Copy(Freeze.TARGET)))
# 	TODO: Make Freeze is an Action
...
class ICC_481:
	...
	def play(self):
		for i in self.controller.field:
			if i.cost<11:
				into = RandomMinion(cost=(i.cost+2)).evaluate(self.controller)
				yield Morph(i, into)
...
class ICC_065:
	...
	"""
	deathrattle = Give(CONTROLLER, "skele11") * 2
...
class ICC_221:
	...
	"""
	play = Lifesteal(FRIENDLY_WEAPON)
...
class ICC_240:
	...
	"""
	update = Find(CURRENT_PLAYER + CONTROLLER) & Refresh(FRIENDLY_WEAPON, {GameTag.IMMUNE: True})
...
class ICC_809:
	...
	"""
	combo = SetTag(TARGET, {GameTag.POISONOUS: True})
...
class ICC_811:
	...
	"""
	play = Morph(FRIENDLY_HAND + SPELL, RandomSpell())
...
class ICC_827e:
	...
	"""
	hand = Play(CONTROLLER).on(Morph(SELF, Copy(Play.CARD)).then(Buff(Morph.CARD, "ICC_827e")))
...
class ICC_827t:
	...
	"""
	class Hand:
		events = Play(CONTROLLER).on(Morph(SELF, Copy(Play.CARD)).then(Buff(Morph.CARD, "ICC_827e")))
...
class ICC_210:
	...
	"""
	events = OWN_TURN_END.on(Buff(RANDOM_OTHER_FRIENDLY_MINION, "ICC_210e"))
...
class ICC_213:
	...
	"""
	play = DISCOVER(RANDOM(FRIENDLY + KILLED + MINION))
...
class ICC_215:
	...
	"""
	play = Shuffle(CONTROLLER, Copy(ENEMY_DECK))
...
class ICC_235:
	...
	https://hearthstone.gamepedia.com/Shadow_Essence
	"""
	play = Summon(CONTROLLER, Copy(RANDOM(FRIENDLY_DECK + MINION))).then(Buff(Summon.CARD, "ICC_235e"))
...
class ICC_830p:
	...
	events = OWN_CARD_PLAY.on(Buff(FRIENDLY_HERO_POWER, {GameTag.HEROPOWER_ADDITIONAL_ACTIVATIONS: 1}))
...
# 	"""
# 	pass
#	TODO: (card.py:714) Make lose divine shield is an action
...
class ICC_068:
	...
	"""
	events = Activate.on(Freeze(Activate.TARGET))
...
class ICC_823:
	...
	"""
	play = Give(CONTROLLER, Copy(LOWEST_ATK(FRIENDLY_HAND + MINION)))
...
class ICC_833:
	...
	"""
	play = Summon(CONTROLLER, "ICC_833t"), buff(SELF, "ICC_833e")
...
class ICC_836:
	...
	"""
	play = Hit(RANDOM(ENEMY_MINIONS), 2).then(Freeze(Hit.TARGET))
...
class ICC_204:
	...
	"""
	def events(self):
		card = RandomSpell(race=CardClass.HUNTER, secret=True).evaluate(self)
		while (self.controller.secrets.contains(card)):
			card = RandomSpell(race=CardClass.HUNTER, secret=True).evaluate(self)
		yield Play(CONTROLLER, SECRET).after(CastSpell(card))
...
class ICC_243:
	...
	"""
	update = Refresh(FRIENDLY_HAND + MINION + DEATHRATTLE, {GameTag.COST: -2})
...
# 	"""
# 	play = Morph(SELF, "ICC_047t"), Buff(SELF, )
...
# 	"""
# 	play = Morph(SELF, "ICC_047t"), Buff(SELF, "ICC_047e")
...
class ICC_047t2:
	...
	https://hearthstone.gamepedia.com/Fatespinner
	"""
	deathrattle = Hit(ALL_MINIONS, 3), Buff(FRIENDLY_MINIONS - SELF, "ICC_047e")
...
class ICC_054:
	...
	"""
	play = Summon(CONTROLLER, "ICC_832t4"), (Count(ENEMY_MINIONS) > Count(FRIENDLY_MINIONS)) & CastSpell("ICC_054")
...
class ICC_079:
	...
	"""
	play = Buff(FRIENDLY_HERO, "ICC_079e"), GainArmor(CONTROLLER, 3)
...
class ICC_085:
	...
	"""
	play = Hit(TARGET, 5), Draw(CONTROLLER) * 5, GainArmor(CONTROLLER, 5), Summon(CONTROLLER, "ICC_085t")
...
class ICC_832:
	...
	play = ChooseBoth(CONTROLLER) & (Summon(CONTROLLER, "ICC_832t3") * 2, Summon(CONTROLLER, "ICC_832t4") * 2)
...
class ICC_835:
	...
	"""
	deathrattle = Summon(CONTROLLER, Copy(FRIENDLY + KILLED + MINION + TAUNT))
...
class ICC_314t2:
	...
	https://hearthstone.gamepedia.com/Army_of_the_Dead
	"""
	play = (Remove(FRIENDLY_DECK[:1]).then(Find(Remove.TARGET + MINION) & Summon(CONTROLLER, Remove.TARGET))) * 5
...
class ICC_314t3:
	...
	"""
	play = Destroy(ALL_MINIONS), Remove(FRIENDLY_DECK[:1]) * Count(ALL_MINIONS)
...
class ICC_314t4:
	...
	"""
	# TODO: Modified Steal action, add zone to Steal
	play = Steal(RANDOM(ENEMY_DECK)).then(ForceDraw(Steal.TARGET))
...
class ICC_314t5:
	...
	"""
	play = Find(TARGET + FRIENDLY) & Heal(TARGET, 5), Find(TARGET + ENEMY) & Hit(TARGET, 5)
...
class ICC_314t6:
	...
	"""
	play = Destroy(TARGET), Hit(FRIENDLY_HERO, CURRENT_HEALTH(TARGET))
