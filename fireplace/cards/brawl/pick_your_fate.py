...
class TB_PickYourFateRandom:
    ...
    events = OWN_TURN_BEGIN.on(GenericChoice(CONTROLLER, RandomFate * 4), Destroy(SELF))
...
class TB_PickYourFate_2nd:
    ...
    events = OWN_TURN_BEGIN.on(
        (CURRENT_HEALTH(FRIENDLY_HERO) <= 20)
...
class TB_PickYourFate_2_Ench:
    events = Summon(CONTROLLER, MINION).on(
        Buff(Summon.CARD, "TB_PickYourFate_2_EnchMinion")
    )
...
class TB_PickYourFate_4_Ench:
    events = Summon(CONTROLLER, MINION).on(
        Buff(Summon.CARD, "TB_PickYourFate_4_EnchMinion")
    )
...
class TB_PickYourFate_7_EnchMiniom2nd:
    deathrattle = Buff(
        RANDOM(FRIENDLY_HAND + (COST > 0)), "TB_PickYourFate_7_EnchMiniom2nde"
    )
...
class TB_PickYourFate_9_Ench:
    update = Refresh(
        FRIENDLY_MINIONS + DEATHRATTLE, buff="TB_PickYourFate_9_EnchMinion"
    )
